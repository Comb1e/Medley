...
TEXTING_SKILL_PATH = "E:/Projects/Agent/skills/CODING.md"

# ====== use LLM for texting =====
def generate_text(input_raw):
    ...
    result = texing_llm.invoke()

    try:
        result_dict = ast.literal_eval(result)
    except:
        print(result)
        raise ValueError("Invalid input format. Please provide a valid list of {file_path, text}.")
...
generate_text.name = "generate_text"
generate_text.description = (
    "#Must use get_prompt(raw_prompt) tool to generate the prompt parameters and save them in a JSON file. Then input the file path of the JSON file to this tool to generate text.#"
    "#Must use this tool when you need text.#"
    "Create a file in the specified location on the computer and write text."
    "The input is a string containing the path of the JSON file with the prompt parameter."
    "The output is an array containing multiple items, each is the path to the text file just generated ."
)
generate_text.input = {
    "file_dir": str
}
