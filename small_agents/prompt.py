...
def get_prompt(raw_prompt):
    ...
    get_prompt_agent = agent(
        ...
        tools = prompt_tools,
        main_distinctions= f"1. The parameter \"path\" should be the path indicated by the user in #Raw Prompt#. If the user did not indicate one, it should be {config.GENERATE_PATH}.",
        temperature = 0
    )
